...
const TOTAL_SMP = 133;
const NEGERI_COUNT = 40;
const SWASTA_COUNT = TOTAL_SMP - NEGERI_COUNT;

let schools = [];

// Generate SMP Negeri
for (let i = 1; i <= NEGERI_COUNT; i++) {
    // Basic NPSN format for SMP: 208xxxx or 108xxxx. We'll use 20800000 + i
    let npsn = (20803000 + i).toString();
    schools.push({
        name: `SMPN ${i} PESAWARAN`,
        npsn: npsn,
        status: "Negeri"
    });
}
...
// Generate SMP Swasta
const swastaNames = ["MUHAMMADIYAH", "AL KAUTSAR", "PGRI", "TAMAN SISWA", "BINA MANDIRI", "DARUL HUFFAZ", "MATHLAUL ANWAR", "IT", "KATHOLIK", "PELITA", "BINA INSANI", "NURUL IMAN"];
for (let i = 1; i <= SWASTA_COUNT; i++) {
    let npsn = (69003000 + i).toString();
    let nameSuffix = swastaNames[i % swastaNames.length];
    schools.push({
        name: `SMPS ${nameSuffix} ${i}`,
        npsn: npsn,
        status: "Swasta"
    });
}
...
const destPath = path.join(__dirname, 'sql', 'seed_smp_pesawaran.sql');
