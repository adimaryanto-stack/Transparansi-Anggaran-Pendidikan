...
const TOTAL_PAUD = 428;
const NEGERI_COUNT = 50; // TK Negeri Pembina dsb
const SWASTA_COUNT = TOTAL_PAUD - NEGERI_COUNT;

let schools = [];

// Generate PAUD / TK Negeri
for (let i = 1; i <= NEGERI_COUNT; i++) {
    // NPSN format for TK/PAUD Negeri
    let npsn = (69700000 + i).toString();
    schools.push({
        name: `TK NEGERI PEMBINA ${i} PESAWARAN`,
        npsn: npsn,
        status: "Negeri"
    });
}

// Generate PAUD / TK / KB Swasta
const swastaNames = ["KB KASIH IBU", "TKIT BINA INSANI", "TK MUHAMMADIYAH", "TK AISYIYAH", "KB TUNAS BANGSA", "SPS HARAPAN", "TPA PERMATA HATI", "TK DHARMA WANITA", "KB MUTIARA", "TK AL QURAN", "KB MELATI", "TK PERTIWI"];
for (let i = 1; i <= SWASTA_COUNT; i++) {
    // NPSN format for TK/PAUD Swasta
    let npsn = (69900000 + i).toString();
...
const sqlTemplate = `-- ==============================================================================
-- Script Seed: 428 PAUD (Negeri & Swasta) di Kabupaten Pesawaran
-- Deskripsi: Menambahkan 428 PAUD beserta pembuatan Akun (User & Profile).
...
const destPath = path.join(__dirname, 'sql', 'seed_paud_pesawaran.sql');
