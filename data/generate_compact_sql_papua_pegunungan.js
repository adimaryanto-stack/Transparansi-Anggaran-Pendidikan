...
function run() {
    if (!fs.existsSync(jsonPath)) {
        console.error('Error: papua_pegunungan_all_schools.json not found. Wait for the scraper to finish.');
        process.exit(1);
    }
    ...
    // Group into batches of 100 (approx 10-15 batches depending on the total)
    ...
    // Create directory data/sql/compact_papua_pegunungan
    const sqlDir = path.join(__dirname, 'sql', 'compact_papua_pegunungan');
    ...
    batches.forEach((batch, idx) => {
        ...
        const jsonStr = '[\n' + lines.join(',\n') + '\n]';
        const escapedJsonStr = jsonStr.replace(/'/g, "''"); // escape single quotes in JSON string for SQL
        
        let sql = `-- Compact Batch ${idx + 1} of ${batches.length} (Papua Pegunungan)\n`;
        sql += `DO $$\n`;
        sql += `DECLARE\n`;
        ...
        sql += `        x.name, \n`;
        sql += `        x.npsn, \n`;
        sql += `        concat_ws(', ', nullif(x.address, ''), nullif(x.village, ''), nullif(x.district, ''), nullif(x."regencyName", ''), 'Papua Pegunungan'),\n`;
        sql += `        'C',\n`;
        sql += `        r.id\n`;
