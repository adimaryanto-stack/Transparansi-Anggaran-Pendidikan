...
// Target provinces configuration
const TARGET_PROVINCES = [
  { name: 'Aceh', kemendikbudCode: '060000', bpsProvinceCode: '11' },
  { name: 'Kepulauan Riau', kemendikbudCode: '310000', bpsProvinceCode: '21' },
  { name: 'Sulawesi Utara', kemendikbudCode: '170000', bpsProvinceCode: '71' },
  { name: 'Maluku', kemendikbudCode: '210000', bpsProvinceCode: '81' },
  { name: 'Maluku Utara', kemendikbudCode: '270000', bpsProvinceCode: '82' },
  { name: 'Nusa Tenggara Timur', kemendikbudCode: '240000', bpsProvinceCode: '53' }
];
...
function normalize(name) {
  ...
  const clean = lower
      .replace(/\b(kab|kabupaten|kota)\.?\b/gi, '')
      .replace(/[^a-z0-9]/g, '')
      .trim();
  return `${type}_${clean}`;
}
...
async function processProvince(prov) {
  ...
  for (let idx = 0; idx < batches.length; idx++) {
    ...
    if (rpcError) {
      console.error(`  ❌ Error executing batch ${idx + 1}:`, rpcError);
      process.exit(1);
    } else {
...
async function main() {
  console.log('🚀 Starting Unified Pipeline for 6 Provinces...');
  for (const prov of TARGET_PROVINCES) {
    await processProvince(prov);
  }
  console.log('\n🎉 All 6 provinces processed, seeded, and synced successfully!');
}
