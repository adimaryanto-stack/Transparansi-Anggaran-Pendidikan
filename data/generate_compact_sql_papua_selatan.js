...
function run() {
    if (!fs.existsSync(jsonPath)) {
        console.error('Error: papua_selatan_all_schools.json not found. Wait for the scraper to finish.');
        process.exit(1);
    }
    ...
    // Create directory data/sql/compact_papua_selatan
    const sqlDir = path.join(__dirname, 'sql', 'compact_papua_selatan');
    ...
    batches.forEach((batch, idx) => {
        ...
        let sql = `-- Compact Batch ${idx + 1} of ${batches.length} (Papua Selatan)\n`;
        sql += `DO $$\n`;
        sql += `DECLARE\n`;
        ...
        sql += `        x.name, \n`;
        sql += `        x.npsn, \n`;
        sql += `        concat_ws(', ', nullif(x.address, ''), nullif(x.village, ''), nullif(x.district, ''), nullif(x."regencyName", ''), 'Papua Selatan'),\n`;
        sql += `        'C',\n`;
        sql += `        r.id\n`;
