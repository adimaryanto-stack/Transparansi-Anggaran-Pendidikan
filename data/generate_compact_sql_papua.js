...
function run() {
    if (!fs.existsSync(jsonPath)) {
        console.error('Error: papua_all_schools.json not found. Wait for the scraper to finish.');
        process.exit(1);
    }
    ...
    // Create directory data/sql/compact_papua
    const sqlDir = path.join(__dirname, 'sql', 'compact_papua');
    ...
    batches.forEach((batch, idx) => {
        ...
        let sql = `-- Compact Batch ${idx + 1} of ${batches.length} (Papua)\n`;
        sql += `DO $$\n`;
        sql += `DECLARE\n`;
        ...
        sql += `        x.name, \n`;
        sql += `        x.npsn, \n`;
        sql += `        concat_ws(', ', nullif(x.address, ''), nullif(x.village, ''), nullif(x.district, ''), nullif(x."regencyName", ''), 'Papua'),\n`;
        sql += `        'C',\n`;
        sql += `        r.id\n`;
